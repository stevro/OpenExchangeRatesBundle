...
class Currencies
{
    ...
    /**
     * Searches for a currency by it's ISO code and returns whatever we have available to be used for dislay. The priorities are Symbol($), ASCII (&#36;), isoCode(USD).
     * If nothing found then it returns NULL.
     * @param string $isoCode ISO Code of the currency
     * @return mixed
     */
    public static function getCurrencyForDisplay($isoCode = 'USD')
    {
        $currency = self::getCurrencyByIsoCode($isoCode);

        $currencyDisplay = null;

        if (isset($currency['symbol'])) {
            $currencyDisplay = $currency['symbol'];
        } elseif (isset($currency['ASCII'])) {
            $currencyDisplay = $currency['ASCII'];
        } else {
